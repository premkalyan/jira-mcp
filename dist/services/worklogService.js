...
export class WorklogService {
    ...
    async addWorklog(params) {
        try {
            ...
            const result = await this.apiClient.addWorklog(params.issueKey, params.timeSpent, params.comment, params.startDate);
            return {
                content: [
                    {
                        type: 'text',
...
- View issue details: Use \`get_issue_details\` with issueKey: ${issueKey}`,
            ...
            const tableData = worklogs.map((worklog) => [
            ...
            const markdownTable = formatMarkdownTable(['Author', 'Time Spent', 'Work Date', 'Logged Date', 'Has Comment'], tableData);
            const totalSeconds = worklogs.reduce((sum, w) => sum + w.timeSpentSeconds, 0);
            const totalTime = this.formatDuration(totalSeconds);
            const uniqueAuthors = [...new Set(worklogs.map((w) => w.author.displayName))];
            return {
                content: [
                    {
                        type: 'text',
...
- **Latest Entry**: ${new Date(Math.max(...worklogs.map((w) => new Date(w.created).getTime()))).toLocaleString()}
- **Earliest Entry**: ${new Date(Math.min(...worklogs.map((w) => new Date(w.created).getTime()))).toLocaleString()}
