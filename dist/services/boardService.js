    ...
    async getBoards(params = {}) {
        try {
            ...
            if (params.projectKey)
                apiParams.projectKeyOrId = params.projectKey;
            ...
            const markdownTable = formatMarkdownTable(['ID', 'Name', 'Type', 'Project Key', 'Project Name'], tableData);
            return {
                content: [
                    {
                        type: 'text',
...
- Search issues: Use \`search_issues\` with JQL: \`project = "${board.location?.projectKey}"\``;
    ...
    async getBoardIssues(params) {
        try {
            ...
            const board = await this.apiClient.getBoard(params.boardId);
            ...
            if (params.assigneeFilter)
                filterSummary.push(`Assignee: ${params.assigneeFilter}`);
            if (params.statusFilter)
                filterSummary.push(`Status: ${params.statusFilter}`);
