...
## Changes Applied
${Object.keys(sprintData).map(key => `- Updated **${key}**: ${sprintData[key]}`).join('\n')}`,
                    },
                ],
            };
        }
    ...
    async getSprintDetails(sprintId) {
        try {
            ...
            const markdownTable = issues.length > 0
                ? formatMarkdownTable(['Key', 'Summary', 'Status', 'Assignee', 'Type'], tableData)
    ...
    async getBoardSprints(boardId, state) {
        try {
            ...
            const markdownTable = sprints.length > 0
                ? formatMarkdownTable(['ID', 'Name', 'State', 'Start Date', 'End Date', 'Goal'], tableData)
...
- **Add More Issues**: Use \`add_issues_to_sprint\` with additional issue keys`,
    ...
    async removeIssuesFromSprint(params) {
        try {
...
- **Add Back to Backlog**: Issues are now available in the backlog`,
    ...
    async moveIssuesBetweenSprints(params) {
        try {
...
- **Complete Sprint**: Use \`complete_sprint\` when done`,
    ...
    async completeSprint(sprintId, incompleteIssuesAction) {
        try {
            ...
            const issues = sprintIssues.issues || [];
            const completedIssues = issues.filter(issue => issue.fields.status.statusCategory.key === 'done');
            const incompleteIssues = issues.filter(issue => issue.fields.status.statusCategory.key !== 'done');
            // Complete the sprint
    ...
    async getSprintCapacity(sprintId) {
        try {
            ...
            const markdownTable = Object.keys(assigneeCapacity).length > 0
                ? formatMarkdownTable(['Assignee', 'Total Issues', 'Completed', 'Total Points', 'Completed Points', 'Progress'], capacityTable)
...
${completedStoryPoints / totalStoryPoints < 0.3 && sprint.state === 'active'
    ...
    // 12. Set Sprint Goal
    async setSprintGoal(sprintId, goal) {
        try {
