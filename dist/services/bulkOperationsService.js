...
export class BulkOperationsService {
    ...
    async bulkCreateIssues(params) {
        try {
            ...
            const totalStoryPoints = params.issuesData.reduce((sum, issue) => sum + (issue.storyPoints || 0), 0);
            ...
            if (result.issues && result.issues.length > 0) {
                const tableData = result.issues.map((issue, index) => [
                    issue.key,
                ...
                issuesTable = formatMarkdownTable(['Key', 'Summary', 'Type', 'Priority', 'Story Points'], tableData);
            }
            return {
                content: [
                    {
        ...
        try {
            ...
            const updateTypes = params.updates.reduce((acc, update) => {
                ...
                if (update.storyPoints)
                    fields.push('storyPoints');
                fields.forEach(field => {
                    acc[field] = (acc[field] || 0) + 1;
                });
                return acc;
            }, {});
            const totalStoryPoints = params.updates.reduce((sum, update) => sum + (update.storyPoints || 0), 0);
            const issuesTable = params.updates.map(update => {
                ...
                if (update.storyPoints)
                    updatedFields.push('Story Points');
                return [
                    update.issueKey,
            ...
            const markdownTable = formatMarkdownTable(['Issue Key', 'Updated Fields', 'Story Points', 'Priority'], issuesTable);
            return {
                content: [
                    {
                        type: 'text',
...
- **Progress Tracking**: Monitor updated issues through development lifecycle`,
    ...
    async bulkTransitionIssues(params) {
        try {
            ...
            const result = await this.apiClient.bulkTransitionIssues(params.issueKeys, params.transitionName, params.comment);
            // Analyze results
            const successful = result.results.filter((r) => r.status === 'success');
            const failed = result.results.filter((r) => r.status === 'error');
            const resultsTable = result.results.map((result) => [
                result.issueKey,
                result.status === 'success' ? '✅ Success' : '❌ Failed',
                result.status === 'success' ? params.transitionName : result.message,
            ]);
            const markdownTable = formatMarkdownTable(['Issue Key', 'Status', 'Result/Error'], resultsTable);
            return {
                content: [
                    {
                        type: 'text',
...
${failed.map((f) => `- **${f.issueKey}**: ${f.message}`).join('\n')}
...
${successful.length > 0 ? '- **Monitor Progress**: Track issues through remaining workflow states' : ''}`,
    ...
    async bulkAssignIssues(issueKeys, assignee) {
        try {
