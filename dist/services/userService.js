...
- Check spelling and try variations`,
            ...
            const markdownTable = formatMarkdownTable(['Display Name', 'Email', 'Status', 'Time Zone', 'Account ID (truncated)'], tableData);
...
${formattedUsers.map(user => `- **${user.displayName}**: \`${user.accountId}\``).join('\n')}`,
    ...
    async getUserDetails(accountId) {
        try {
            ...
            const user = await this.apiClient.getUser(accountId);
            return {
                content: [
                    {
                        type: 'text',
