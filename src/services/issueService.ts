...
- Transition issue: Use \`transition_issue\``,
  ...
  async setStoryPoints(issueKey: string, storyPoints: number): Promise<ToolResult> {
    try {
...
- **Track Velocity**: Story points now contribute to velocity metrics`,
  ...
  async getStoryPoints(issueKey: string): Promise<ToolResult> {
    try {
...
- **Capacity Analysis**: Use \`get_sprint_capacity\` to see sprint impact`,
  ...
  async bulkUpdateStoryPoints(updates: Array<{ issueKey: string; storyPoints: number }>): Promise<ToolResult> {
    try {
...
- **Create More Subtasks**: Use \`create_subtask\` to further break down work`,
  ...
  async getSubtasks(parentIssueKey: string): Promise<ToolResult> {
    try {
